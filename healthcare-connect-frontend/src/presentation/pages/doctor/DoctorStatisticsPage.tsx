...
const DoctorStatisticsPage = () => {
    ...
    const StatCard = ({ icon, title, value, change, unit = '' }: {
        icon: string;
        title: string;
        value: number | string;
        change?: number;
    ...
    const RatingBar = ({ stars, percentage, count }: { stars: number; percentage: number; count: number }) => (
        <div className="flex items-center gap-3">
            <div className="w-16 text-sm font-medium text-gray-600 dark:text-gray-400">{stars} ★</div>
            <div className="flex-1 h-6 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                <div className="h-full bg-yellow-400 rounded-full transition-all duration-500" style={{ width: `${percentage}%` }} />
            </div>
            <div className="w-24 text-sm text-gray-500 dark:text-gray-400 text-right">{percentage}% ({count})</div>
        </div>
    );
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-cyan-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
            ...
            <div className="relative z-10 container mx-auto px-4 py-6">
                {/* Header - LUÔN HIỂN THỊ, KHÔNG LOADING */}
                <DashboardHeader
                    icon="📊"
                    title={t('doctor.stats.title')}
                    subtitle={t('doctor.stats.subtitle') || 'Thống kê số liệu khám bệnh của bạn'}
                    showHospital={true}
                    hospitalName={doctorInfo?.hospitalName || (dataLoading ? 'Đang tải...' : '')}
                />

                {/* Filter + Export Buttons - LUÔN HIỂN THỊ */}
                <div className="bg-white/60 dark:bg-gray-800/40 backdrop-blur-sm rounded-xl p-4 mb-6 flex flex-wrap justify-between items-center gap-4">
                    ...
                    <div className="flex gap-2">
                        <Button variant="outline" size="sm" onClick={handleExportExcel} disabled={dataLoading || !stats}>
                            📥 {t('doctor.stats.exportExcel')}
                        </Button>
                        <Button variant="outline" size="sm" onClick={handleExportPDF} loading={exporting} disabled={dataLoading || !stats}>
            ...
            <style>{`
                @media print {
                    ...
                    .relative.z-10, .relative.z-10 * { visibility: visible; }
                    .relative.z-10 { position: absolute; top: 0; left: 0; right: 0; }
                    button, .bg-white\\/80, .backdrop-blur-sm { background: white !important; backdrop-filter: none !important; }
