...
import type { AdminDoctorListResponse, HospitalResponse } from '../../../core/types/api.response';
...
import ApproveDoctorModal from './components/ApproveDoctorModal';
...
const statusOptions = (t: (key: string) => string): { value: StatusFilter; label: string; icon: string; color: string }[] => [
    { value: 'ALL', label: t('admin.doctors.statusAll'), icon: '📋', color: 'bg-gray-100 text-gray-700' },
    { value: 'PENDING', label: t('admin.doctors.statusPending'), icon: '⏳', color: 'bg-yellow-100 text-yellow-700' },
    { value: 'VERIFIED', label: t('admin.doctors.statusVerified'), icon: '🟡', color: 'bg-blue-100 text-blue-700' },
    { value: 'APPROVED', label: t('admin.doctors.statusApproved'), icon: '✅', color: 'bg-green-100 text-green-700' },
    { value: 'REJECTED', label: t('admin.doctors.statusRejected'), icon: '❌', color: 'bg-red-100 text-red-700' },
];
...
const AdminDoctorsPage = () => {
    ...
    // Table loading state (chỉ loading riêng table)
    const [tableLoading, setTableLoading] = useState(true);
    const [tableError, setTableError] = useState<string | null>(null);

    // Modal states
    const [detailModal, setDetailModal] = useState<{ open: boolean; doctorId: string | null }>({
        open: false,
        doctorId: null,
    });
    const [approveModal, setApproveModal] = useState<{ open: boolean; doctorId: string | null; doctorName: string }>({
        open: false,
        doctorId: null,
        doctorName: '',
    });
    const [rejectModal, setRejectModal] = useState<{ open: boolean; doctorId: string | null; doctorName: string }>({
        open: false,
        doctorId: null,
        doctorName: '',
    });
    ...
    const updateUrl = useCallback((page: number, keyword: string, status: StatusFilter, hospitalId: string) => {
        ...
        navigate(`/admin/doctors?${params.toString()}`, { replace: true });
    }, [navigate]);

    // Fetch doctors (chỉ fetch table data)
    ...
    const handleViewDetail = (doctorId: string) => {
        setDetailModal({ open: true, doctorId });
    };

    const handleApprove = (doctorId: string, doctorName: string) => {
        setApproveModal({ open: true, doctorId, doctorName });
    };

    const handleReject = (doctorId: string, doctorName: string) => {
        setRejectModal({ open: true, doctorId, doctorName });
    };

    const confirmApprove = async () => {
        if (!approveModal.doctorId) return;
        setApprovingId(approveModal.doctorId);
        try {
            await adminApi.approveDoctor(approveModal.doctorId);
            toast.success(t('admin.doctors.approveSuccess'));
            fetchDoctors();
            setApproveModal({ open: false, doctorId: null, doctorName: '' });
        } catch (error: any) {
            toast.error(error.response?.data?.message || t('admin.doctors.approveError'));
        } finally {
            setApprovingId(null);
        }
    };
    ...
    const confirmReject = async (reasonCode: string, note: string) => {
        ...
        setRejectingId(rejectModal.doctorId);
        try {
            await adminApi.rejectDoctor(rejectModal.doctorId, reasonCode, note);
            toast.success(t('admin.doctors.rejectSuccess'));
            fetchDoctors();
            setRejectModal({ open: false, doctorId: null, doctorName: '' });
        } catch (error: any) {
            toast.error(error.response?.data?.message || t('admin.doctors.rejectError'));
        } finally {
            setRejectingId(null);
        }
    };
        ...
        if (tableError) {
            return (
                <div className="text-center py-12">
                    ...
                    <button onClick={fetchDoctors} className="mt-3 text-blue-500 hover:underline">{t('common.retry')}</button>
                </div>
            );
        }
        ...
        if (doctors.length === 0) {
            return (
                <div className="text-center py-12">
                    ...
                    <p className="text-gray-500">{t('admin.doctors.noDoctors')}</p>
                    {activeFilterCount > 0 && (
                        <button onClick={clearAllFilters} className="mt-3 text-blue-500 hover:underline">{t('admin.doctors.clearAllFilters')}</button>
                    )}
                </div>
            );
        }
                ...
                <div className="overflow-x-auto">
                    <table className="w-full">
                        <thead className="bg-gray-50 dark:bg-gray-700/50">
                            <tr>
                                <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700 dark:text-gray-300">{t('admin.doctors.avatar')}</th>
                                <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700 dark:text-gray-300">{t('admin.doctors.doctorCode')}</th>
                                ...
                                <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700 dark:text-gray-300">{t('admin.doctors.specialty')}</th>
                                <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700 dark:text-gray-300">{t('admin.doctors.hospital')}</th>
                                <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700 dark:text-gray-300">{t('schedule.price')}</th>
                        ...
                        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                            {doctors.map((doctor) => (
                                <tr key={doctor.id} className="group hover:bg-gray-50 dark:hover:bg-gray-700/30 transition cursor-pointer" onClick={() => handleViewDetail(doctor.id)}>
                                    ...
                                    <td className="px-4 py-3 text-sm font-mono text-gray-600 dark:text-gray-400">{doctor.doctorCode}</td>
                                    <td className="px-4 py-3">
                                    ...
                                    <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">{doctor.email}</td>
                                    <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">{doctor.specialtyName}</td>
                                    <td className="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">{doctor.hospitalName}</td>
                                    <td className="px-4 py-3 text-sm font-medium text-green-600 dark:text-green-400">{formatPrice(doctor.consultationFee)}</td>
                                    <td className="px-4 py-3">{getStatusBadge(doctor.status, t)}</td>
                                    <td className="px-4 py-3 text-center">
                                        <div className="flex justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity" onClick={(e) => e.stopPropagation()}>
                                            {doctor.status === 'PENDING' && (
                                                <>
                                                    <button
                                                        onClick={() => handleApprove(doctor.id, doctor.fullName)}
                                                        ...
                                                        title={t('admin.doctors.approve')}
                                                    >
                                                        ✅
                                                    </button>
                                                    <button
                                                        onClick={() => handleReject(doctor.id, doctor.fullName)}
                                                        disabled={rejectingId === doctor.id}
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-cyan-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
            <div className="container mx-auto px-4 py-6">
                ...
                {/* Filters - Luôn hiển thị, không loading */}
                    ...
                    <div className="p-5">
                        <div className="flex flex-col lg:flex-row gap-3">
                            ...
                            <div className="w-full lg:w-48">
                                <select
                                    ...
                                    className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white"
                                >
                                    <option value="">🏥 {t('admin.doctors.allHospitals')}</option>
                        ...
                        {activeFilterCount > 0 && (
                            <div className="flex flex-wrap items-center gap-2 mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                                <span className="text-xs text-gray-400">{t('admin.doctors.activeFilters')}:</span>
                                {searchKeyword && (
                                ...
                                {statusFilter !== 'ALL' && (
                                    <span className="inline-flex items-center gap-1 px-2 py-1 text-xs bg-purple-100 text-purple-700 rounded-full">
                                        {statusOptionsList.find(s => s.value === statusFilter)?.icon} {statusOptionsList.find(s => s.value === statusFilter)?.label}
                                        <button onClick={() => handleStatusFilterChange('ALL')} className="ml-1 hover:text-purple-900">✕</button>
                                    </span>
                                )}
                                ...
                                <button onClick={clearAllFilters} className="text-xs text-red-500 hover:text-red-600 flex items-center gap-1">
                                    🗑️ {t('admin.doctors.clearAllFilters')}
                                </button>
                            </div>
                        )}
                    </div>
                ...
                {/* Doctors Table - Chỉ loading phần này */}
                <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-sm overflow-hidden">
                    <div className="px-5 py-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
                        <h2 className="font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                            📋 {t('admin.doctors.list')}
                            <span className="text-sm font-normal text-gray-500">({totalElements} {t('admin.doctors.doctors')})</span>
                        </h2>
                        {!tableLoading && doctors.length > 0 && (
            ...
            <DoctorDetailModal
                ...
                doctorId={detailModal.doctorId}
                onClose={() => setDetailModal({ open: false, doctorId: null })}
            />

            <ApproveDoctorModal
                isOpen={approveModal.open}
                doctorName={approveModal.doctorName}
                onClose={() => setApproveModal({ open: false, doctorId: null, doctorName: '' })}
                onConfirm={confirmApprove}
                loading={approvingId === approveModal.doctorId}
            />

            <RejectDoctorModal
                isOpen={rejectModal.open}
                doctorName={rejectModal.doctorName}
                onClose={() => setRejectModal({ open: false, doctorId: null, doctorName: '' })}
                onConfirm={confirmReject}
                loading={rejectingId === rejectModal.doctorId}
            />
        </div>
    );
};
