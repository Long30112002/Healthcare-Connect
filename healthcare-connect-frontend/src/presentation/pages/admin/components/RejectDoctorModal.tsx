...
const RejectDoctorModal = ({ isOpen, doctorName, onClose, onConfirm, loading = false }: RejectDoctorModalProps) => {
  ...
  return (
    <Modal
      ...
      title={t('admin.doctors.rejectTitle')}
      variant="danger"
    ...
    >
      <div className="space-y-4">
        <p className="text-sm text-gray-600 dark:text-gray-400">
          {t('admin.doctors.rejectConfirmMessage', { name: doctorName })}
        </p>
        
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            {t('admin.doctors.rejectReason')} <span className="text-red-500">*</span>
          </label>
          <select
            value={rejectReason}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
            {t('admin.doctors.rejectNote')} <span className="text-gray-400 text-xs">({t('common.optional')})</span>
          </label>
          <textarea
            value={rejectNote}
            ...
            placeholder={t('admin.doctors.rejectNotePlaceholder')}
          />
          <p className="text-xs text-gray-400 mt-1">
            {t('admin.doctors.rejectNoteHint')}
          </p>
        </div>
      </div>
    </Modal>
