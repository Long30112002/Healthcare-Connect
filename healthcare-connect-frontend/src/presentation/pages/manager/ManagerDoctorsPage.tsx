...
import { approveDoctor, getDoctorsByManager, rejectDoctor } from '../patient/managerApi';
...
const ManagerDoctorsPage = () => {
  ...
  useEffect(() => {
    const fetchDoctors = async () => {
      ...
      try {
        ...
        const response: PageResponse<DoctorResponse> = await getDoctorsByManager(currentPage, 10, statusParam);
  ...
  const handleApprove = async (doctorId: string) => {
    ...
    try {
      ...
      // Refresh list
      const statusParam = statusFilter === 'ALL' ? undefined : statusFilter;
      const response = await getDoctorsByManager(currentPage, 10, statusParam);
      setDoctors(response.content);
      setTotalPages(response.totalPages);
      setTotalElements(response.totalElements);
    } catch (error) {
  ...
  const handleConfirmReject = async () => {
    ...
    try {
      await rejectDoctor(doctorId, { reasonCode: rejectReason, note: rejectNote });
